...
def get_qr_for_user(bale_chat_id, phone_number=None, service_url=None):
    """
    گرفتن QR برای یک کاربر Bale - فوق ساده
    ورودی: bale_chat_id (مثلا 123456789) و شماره اختیاری
    خروجی: {ok, connected, qr, qrImage, ...}
    """
    if not service_url:
        service_url = DEFAULT_SERVICE_URL
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.warning(f"⚠️ get_qr failed: {resp.status_code} {resp.text[:200]}")
            return {"ok": False, "error": f"HTTP {resp.status_code}"}
    except Exception as e:
        logger.error(f"❌ get_qr exception: {e}")
